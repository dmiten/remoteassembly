...
describe('CounterState', () => {
  ...
  describe('increment', () => {
    ...
    it('should increment the value property by one', () => {
      const [secondState] = dispatch(
        initialState,
        CounterStateActions.increment()
      )
      expect(getValue(secondState)).toBe(getValue(initialState) + 1)

      const [thirdState] = dispatch(
        secondState,
        CounterStateActions.increment()
      )
  ...
  describe('random', () => {
    const [nextState, effects] = dispatch(
      initialState,
      CounterStateActions.random()
    )
